...
// Tillägg: Historikvy per anställd (tryck på en anställd för att se alla lönespecar)
              ...
              {[...history].reverse().map(r => (
                <div key={r.month} className="p-4">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      <p className="text-slate-500 text-xs">Brutto</p>
                      <p className="text-slate-300 text-xs font-medium">{kr(r.grossSalary)}</p>
                    </div>
                    <div>
                      <p className="text-slate-500 text-xs">Skatt</p>
                      <p className="text-slate-300 text-xs font-medium">{kr(r.incomeTax)}</p>
                    </div>
                    <div>
                      <p className="text-slate-500 text-xs">Avg.</p>
                      <p className="text-slate-300 text-xs font-medium">{kr(r.employerFee)}</p>
                    </div>
                    {r.sickPay > 0 && (
                      <div>
                        <p className="text-slate-500 text-xs">Sjuklön</p>
                        <p className="text-amber-400 text-xs font-medium">{kr(r.sickPay)}</p>
                      </div>
                    )}
                    {r.overtimePay > 0 && (
                      <div>
                        <p className="text-slate-500 text-xs">Övertid</p>
                        <p className="text-blue-400 text-xs font-medium">{kr(r.overtimePay)}</p>
                      </div>
                    )}
                    {r.vacationAllowance > 0 && (
                      <div>
                        <p className="text-slate-500 text-xs">Semester</p>
                        <p className="text-purple-400 text-xs font-medium">{kr(r.vacationAllowance)}</p>
                      </div>
                    )}
                  </div>
                </div>
          ...
          {employees.map((emp) => {
            ...
            return (
              <div key={emp.id} className="glass-card p-4">
                ...
                {/* Redigera/radera-knappar */}
                <div className="flex gap-2 mt-3 pt-3 border-t border-slate-700/40">
                  <button
                    onClick={() => onEdit(emp)}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-slate-700/50 text-slate-400 text-xs font-medium"
                  >
                    <Edit3 size={13} /> Redigera
                  </button>
                  <button
                    onClick={() => onDelete(emp.id)}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-red-500/10 text-red-400 text-xs font-medium"
                  >
...
function SettingsPage({ employer, onSave }: { employer: Employer | null; onSave: (e: Employer) => void }) {
  ...
  return (
    <div className="overflow-y-auto h-full px-4 pb-28" style={{ WebkitOverflowScrolling: 'touch' }}>
      ...
      <button onClick={handleSave} className={`w-full py-4 rounded-2xl font-semibold text-sm transition-all ${
        saved ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-blue-500 text-white'
      }`}>{saved ? '✓ Sparat!' : 'Spara inställningar'}</button>
  ...
  return (
    <div style={{ backgroundColor: '#0a0f1e', minHeight: '100dvh', display: 'flex', flexDirection: 'column' }}>
      <main style={{ flex: 1, position: 'relative', overflow: 'hidden', height: 'calc(100dvh - 65px)' }}>
        {page === 'dashboard' && (
          <DashboardPage employees={employees} onRunPayroll={() => setModal('runPayroll')}
            onGoToEmployees={goToEmployees} onDeletePayroll={handleDeletePayroll} />
        )}
        {page === 'employees' && (
          <EmployeesPage employees={employees}
            ...
            onHistory={e => { setHistoryEmployee(e); setModal('employeeHistory') }}
          />
        )}
        {page === 'payroll' && (
          <DashboardPage employees={employees} onRunPayroll={() => setModal('runPayroll')}
